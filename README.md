...
协议要求响应时限不大于解决时限，已用于工单的协议不能覆盖改价式改写。每分钟巡检未结工单，超时只记录一次；人工响应/解决时也会同步检查，避免漏记。自动和人工升级均生成独立记录。升级事件须由管理员分派给本企业有效账号，记录处置要求；工单解决后，由不同审核人员复核关闭升级事件。所有升级事件完成前，工单不能结案。重新打开工单会开启新一轮计时并保留旧审计记录。

## 已交付的页面与接口

业务端有协议、工单、超时事件和升级记录列表，支持搜索、筛选、详情、操作轨迹与 CSV 导出；管理端可管理账号、角色并查看审计。后端提供事务、租户隔离、幂等键、并发版本和附件鉴权。

业务概览新增“服务时限风险”清单，可切换未来 4、24、72 小时窗口，区分首次响应与问题解决节点，显示截止时间、剩余分钟和已超时状态。对应只读接口为 `GET /api/sla/risks?withinHours=4`；范围仅限当前租户且仅包含未解决工单，查询不会写入超时流水或审计。
